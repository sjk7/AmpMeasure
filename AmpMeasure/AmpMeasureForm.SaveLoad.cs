    ...
    public partial class AmpMeasureMain
    {
        ...
        private async Task<bool> SaveWorksheetAsync(string filename, bool silent = false)
        {
            ...
            try
            {
                ...
                _profiler.StartTimer("Save: Collect data from grid");
                var data = GetWorksheetData();
                _profiler.StopTimer("Save: Collect data from grid");

                _profiler.StartTimer("Save: Write XML to disk");
                await Task.Run(() => 
                {
                ...
                _profiler.StopTimer("Save: Write XML to disk");
        ...
        private async Task LoadWorksheetAsync(string filename)
        {
            try
            {
                ...
                _profiler.StartTimer("Load: Read and parse XML");
                await Task.Run(() => 
                {
                    XmlSerializer serializer = new XmlSerializer(typeof(WorksheetData));
                    using (FileStream fs = new FileStream(filename, FileMode.Open, FileAccess.Read, FileShare.Read, 65536))
                    using (BufferedStream bs = new BufferedStream(fs, 65536))
                ...
                _profiler.StopTimer("Load: Read and parse XML");

                _profiler.StartTimer("Load: Populate virtual store");
                ApplyWorksheetData(data);
                _profiler.StopTimer("Load: Populate virtual store");
        ...
        private void ApplyWorksheetData(WorksheetData data)
        {
            ...
            // Pre-allocate virtual data store capacity
            _virtualDataStore.Capacity = Math.Max(_virtualDataStore.Capacity, totalRows);
