...
namespace AmpMeasure
{
    public partial class AmpMeasureMain
    {
        ...
        private bool CheckUnsavedChanges()
        {
            ...
            string worksheetName = string.IsNullOrEmpty(_currentFilePath) 
                ? "Untitled" 
                : Path.GetFileName(_currentFilePath);

            var result = MessageBox.Show(
                $"Do you want to save changes to '{worksheetName}'?", 
                "Unsaved Changes", 
        ...
        private void viewPerformanceLogToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string logPath = PerformanceProfiler.GetLogFilePath();
            ...
            if (File.Exists(logPath))
            {
                ...
                {
                    MessageBox.Show($"Failed to open performance log:\n{ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
                MessageBox.Show($"Performance log file not found.\n\n" +
                    $"The log is created when you:\n" +
                    $"  • Save a worksheet (File > Save or Save As)\n" +
                    $"  • Load a worksheet (File > Open)\n" +
                    $"  • Generate test data (View > Generate Test Data)\n\n" +
                    $"Try saving or loading a file first!\n\n" +
                    $"Expected location:\n{logPath}", 
                    "Performance Log", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
